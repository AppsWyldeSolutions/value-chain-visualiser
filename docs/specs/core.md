...
node: string; nodeOrder: number;
group: 'Primary' | 'Satellite' | 'Transfer Points';
subGroup: string; metric: string;
operator: Operator; valueType: string; symbol: string;
year: number; period: string;
actual: number | null; budget: number | null;
}
export interface Theme { fillColor: string; shortfallColor: string; /* fonts, radii, glass opacity */ }
...
export function evaluate(row: KpiRow): { met: boolean; level: number; label: string; hasData: boolean };
export function formatValues(row: KpiRow): { actual: string; budget: string; pair: string };
export function buildModel(rows: KpiRow[], sel: { year: number; period: string }): ValueChainVisualiserModel;
